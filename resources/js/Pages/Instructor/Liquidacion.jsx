        ...
        <InstructorLayout user={user}>
            ...
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                ...
                <div style={{ background: 'rgba(10,10,10,0.95)', border: '2px solid rgba(255,20,147,0.3)', borderRadius: 12, padding: '1.25rem 1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                    ...
                    <div>
                        <p style={{ color: '#FF1493', fontWeight: 700, margin: '0 0 0.2rem', fontSize: '1rem' }}>{user.name}</p>
                    ...
                    <div style={{ marginLeft: 'auto', background: tipoTarifa === 'por_asistente' ? 'rgba(147,51,234,0.1)' : 'rgba(255,20,147,0.1)', border: `1px solid ${tipoTarifa === 'por_asistente' ? '#9333ea' : '#FF1493'}`, borderRadius: 20, padding: '0.375rem 1rem', color: tipoTarifa === 'por_asistente' ? '#9333ea' : '#FF1493', fontSize: '0.8rem', fontWeight: 700 }}>
                ...
                <div style={{ background: 'rgba(10,10,10,0.95)', border: '2px solid rgba(255,20,147,0.3)', borderRadius: 12, padding: '1.5rem', marginBottom: '2rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <label style={{ color: '#FF1493', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>Desde</label>
                        <input type="date" value={fechaInicio} onChange={e => setFechaInicio(e.target.value)} style={inp} />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <label style={{ color: '#FF1493', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>Hasta</label>
                    ...
                    {hasData && (
                        <button onClick={limpiar} style={{ background: 'transparent', border: '2px solid rgba(255,20,147,0.4)', color: '#FF1493', padding: '0.625rem 1.5rem', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem' }}>
                            ✕ Limpiar
                        </button>
                    )}
                </div>

                {/* ── BANNER PAGO YA REGISTRADO ─────────────────────────────── */}
                {pagoRegistrado && (
                    <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <p style={{ color: '#999', margin: 0, fontSize: '0.875rem' }}>
                                ...
                                {pagoRegistrado.notas && (
                                    <>&nbsp;·&nbsp;Nota: <em style={{ color: '#aaa' }}>{pagoRegistrado.notas}</em></>
                                )}
                            </p>
                        </div>
                    ...
                    <>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                            {[
                                ...
                                { label: 'Total a Pagar',    value: fmtCOP(totales?.total_pago ?? 0), icon: '💰', color: '#FF1493', big: true  },
                            ].map(s => (
                        ...
                        <div style={{ background: 'rgba(10,10,10,0.95)', border: yaPagado ? '2px solid rgba(34,197,94,0.4)' : '2px solid rgba(255,20,147,0.3)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 0 30px rgba(255,20,147,0.1)' }}>
                            ...
                            <div style={{ overflowX: 'auto' }}>
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    <thead>
                                    <tr style={{ borderBottom: '2px solid rgba(255,20,147,0.2)' }}>
                                        {['Fecha', 'Tipo de Clase', 'Horario', 'Sala', 'Asistentes', 'Tarifa aplicada', 'Subtotal'].map(h => (
                                            <th key={h} style={{ padding: '1rem 1.25rem', color: '#FF1493', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: 1, textAlign: 'left', whiteSpace: 'nowrap' }}>
                                                {h}
                                            </th>
                                        ))}
                                    </tr>
                                    ...
                                    <tbody>
                                    {clases.map((clase, i) => (
                                        <tr key={clase.id} style={{ borderBottom: '1px solid rgba(255,20,147,0.08)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,20,147,0.02)' }}>
                                            ...
                                            <td style={{ padding: '1rem 1.25rem' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                    <div style={{ width: 10, height: 10, borderRadius: '50%', background: clase.tipo_clase?.color ?? '#FF1493' }} />
                                            ...
                                            <td style={{ padding: '1rem 1.25rem', color: '#FF1493', fontSize: '0.9rem', fontWeight: 900, whiteSpace: 'nowrap' }}>
                                                {fmtCOP(clase.pago)}
                                            </td>
                                        </tr>
                                    ))}
                                    ...
                                    <tfoot>
                                    <tr style={{ borderTop: '2px solid rgba(255,20,147,0.3)', background: 'rgba(255,20,147,0.05)' }}>
                                        <td colSpan="6" style={{ padding: '1rem 1.25rem', color: '#FF1493', fontWeight: 900, textAlign: 'right', textTransform: 'uppercase', letterSpacing: 1, fontSize: '0.875rem' }}>
                                            TOTAL A PAGAR
                                        </td>
                                        <td style={{ padding: '1rem 1.25rem', color: '#FF1493', fontWeight: 900, fontSize: '1.25rem' }}>
