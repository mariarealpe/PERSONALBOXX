// ════════════════════════════════════════════════════════════
// PASO 6 — CREAR LA PÁGINA MIS CLASES
// ════════════════════════════════════════════════════════════
//
// La carpeta resources/js/Pages/Instructor/ ya debe existir.
// Crea el archivo NUEVO:
//   resources/js/Pages/Instructor/Clases.jsx
// ────────────────────────────────────────────────────────────
...
export default function InstructorClases({ user, clases, filters }) {
    ...
    const fmtFecha = (dt) => dt
        ? new Date(dt).toLocaleDateString('es-CO', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })
        : '';
    const hora = (dt) => dt
            ...
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                ...
                <div style={{ background: 'rgba(10,10,10,0.95)', border: '2px solid rgba(6,182,212,0.3)', borderRadius: 12, padding: '1.5rem', marginBottom: '2rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <label style={{ color: '#06b6d4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>Fecha</label>
                        <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} style={inp} />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <label style={{ color: '#06b6d4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>Estado</label>
                    ...
                    <button onClick={buscar} style={{ background: 'linear-gradient(135deg,#06b6d4,#0891b2)', border: 'none', color: '#000', padding: '0.625rem 1.5rem', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem' }}>
                        🔍 Filtrar
                    </button>
                ...
                <div style={{ background: 'rgba(10,10,10,0.95)', border: '2px solid rgba(6,182,212,0.3)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 0 30px rgba(6,182,212,0.1)' }}>
                    {clases.data && clases.data.length > 0 ? (
                        <>
                            <div style={{ overflowX: 'auto' }}>
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    ...
                                    {clases.data.map((clase, i) => {
                                        ...
                                        return (
                                            <tr key={clase.id} style={{ borderBottom: '1px solid rgba(6,182,212,0.1)', background: i % 2 === 0 ? 'transparent' : 'rgba(6,182,212,0.02)' }}>
                                                ...
                                                <td style={{ padding: '1rem 1.25rem' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                        <div style={{ width: 10, height: 10, borderRadius: '50%', background: clase.tipo_clase?.color ?? '#06b6d4', flexShrink: 0 }} />
                                                ...
                                                <td style={{ padding: '1rem 1.25rem', textAlign: 'center' }}>
                                                        <span style={{ color: (clase.total_reservas ?? 0) >= clase.capacidad_maxima ? '#ef4444' : '#22c55e', fontWeight: 700, fontSize: '0.875rem' }}>
                                                            {clase.total_reservas ?? 0}
                                                        </span>
                                                    <span style={{ color: '#666', fontSize: '0.875rem' }}> / {clase.capacidad_maxima}</span>
                                                </td>
                                                <td style={{ padding: '1rem 1.25rem', color: '#06b6d4', fontSize: '0.875rem', fontWeight: 700, textAlign: 'center' }}>
                                                    {clase.asistencias_count ?? 0}
                                                </td>
                                                <td style={{ padding: '1rem 1.25rem' }}>
                                                ...
                                                <td style={{ padding: '1rem 1.25rem' }}>
                                                    {(clase.estado === 'programada' || clase.estado === 'en_curso') && (
                                                        <Link
                                                            ...
                                                            style={{ background: 'rgba(6,182,212,0.15)', border: '1px solid #06b6d4', color: '#06b6d4', borderRadius: 6, padding: '0.375rem 0.75rem', fontSize: '0.8rem', fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap' }}
                                                        >
                                                            ✅ Asistencia
                                                        </Link>
                                                    )}
                            ...
                            {/* Paginación */}
                            {clases.last_page > 1 && (
                                <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid rgba(6,182,212,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                                    <span style={{ color: '#666', fontSize: '0.875rem' }}>
                                        Mostrando {clases.from}–{clases.to} de {clases.total} clases
                                    </span>
                                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                        {clases.links.map((link, i) => (
                                            <button
                                                ...
                                                style={{ background: link.active ? '#06b6d4' : 'rgba(6,182,212,0.1)', border: '1px solid rgba(6,182,212,0.3)', color: link.active ? '#000' : '#06b6d4', padding: '0.375rem 0.75rem', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, cursor: link.url ? 'pointer' : 'not-allowed', opacity: link.url ? 1 : 0.4 }}
                                            />
                                        ))}
                                    </div>
                                </div>
